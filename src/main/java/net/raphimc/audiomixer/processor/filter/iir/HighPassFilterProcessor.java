 ...
 */
package net.raphimc.audiomixer.processor.filter.iir;
...
public class HighPassFilterProcessor extends BiquadFilterProcessor<HighPassFilterProcessor.InternalProcessor> {
    ...
    public HighPassFilterProcessor(final float frequency) {
        this.frequency.set(frequency);
    }

    public HighPassFilterProcessor(final float frequency, final float q) {
        this.frequency.set(frequency);
        this.q.set(q);
    }
    ...
    protected static final class InternalProcessor extends BiquadFilterProcessor.InternalProcessor {
        ...
        private void applyParameters(final float frequency, final float q) {
            ...
            final double b0 = (1D + cos) * 0.5D;
            final double b1 = -(1D + cos);
            final double b2 = (1D + cos) * 0.5D;
