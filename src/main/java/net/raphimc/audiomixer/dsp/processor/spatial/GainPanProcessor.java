...
public class GainPanProcessor implements Processor {
    ...
    public void process(final AudioBuffer buffer) {
        ...
        final float leftGain = this.gain * (float) Math.cos(normalizedPan * MathUtil.HALF_PI);
        final float rightGain = this.gain * (float) Math.sin(normalizedPan * MathUtil.HALF_PI);
    ...
    public void setGain(final float gain) {
        if (gain < 0F || gain > 1F) {
            throw new IllegalArgumentException("Gain must be >= 0 and <= 1");
        }
        this.gain = gain;
    }
